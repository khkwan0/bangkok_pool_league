...
const RELEASE_SIGNING_LINE =
  'signingConfig bplKeystorePropertiesFile.exists() ? signingConfigs.release : signingConfigs.debug'
...
function addReleaseSigning(contents) {
  ...
  if (!contents.includes('// BPL_RELEASE_SIGNING')) {
    const propsLoader = `
    // BPL_RELEASE_SIGNING
    def bplKeystoreProperties = new Properties()
    def bplKeystorePropertiesFile = rootProject.file('keystore.properties')
    if (bplKeystorePropertiesFile.exists()) {
        bplKeystoreProperties.load(new FileInputStream(bplKeystorePropertiesFile))
    }
`
    contents = contents.replace(/android\s*\{/, `android {${propsLoader}`)
  }

  if (!contents.includes('BPL_RELEASE_SIGNING_CONFIG')) {
    const releaseConfig = `
        release {
            // BPL_RELEASE_SIGNING_CONFIG
            if (bplKeystorePropertiesFile.exists()) {
                keyAlias bplKeystoreProperties['keyAlias']
                keyPassword bplKeystoreProperties['keyPassword']
                storeFile file(bplKeystoreProperties['storeFile'])
                storePassword bplKeystoreProperties['storePassword']
            }
        }`
